...
def lotesreporte_list(request):
    ...
    for lote in salidas:
        ...
        pc_ci_activas, pc_ci_faltantes = get_conteoplantas_valores("Cosecha","En proceso")
        pc_cf_activas, pc_cf_faltantes = get_conteoplantas_valores("Cosecha","Finalizado")
...
def lotesreporte_list2(request,lote_id):
...
def exportar_excel_generico(request, nombre_modelo):
    ...
    EXPORT_FIELDS = {
        ...
        'lotes': ['lote_code', 'variedad_code','genero','apodo_variedad','cultivo','ubicación','estructura','plantas_padre','plantas_madre','harvest_code','status','siembra_madre','metodo_prod','target','surface','tipo','shipment_hub','as_per_SDCMale','as_per_SDCFemale','observaciones'],
        'variedades': ['variedad_codigo','apodo_variedad', 'cultivo','cod_padre','cod_madre','status'],
        'conteoplantas': ['codigo_lote', 'genero','operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','codigo_madre','codigo_planta','plantas_activas','plantas_faltantes','fecha','camas_incompletas','camas_completas','cocosxcamaincompleta','evento','status','observaciones'],
        'conteosemillas': ['codigo_lote','genero', 'operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','fecha','cantidad_frutos','semillasxfruto','prom_semillasxfruto','nsemana','clasificacion','status','observaciones'],
        'conteofrutosplanilla': ['codigo_lote', 'genero','operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','fecha','cama1','cama2','cama3','cama4','cama5','media','prom_area','prom_general','status','observaciones'],
        'conteofrutos': ['codigo_lote', 'genero','operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','fecha','prom_autopolinizados','prom_floresabiertas','prom_polinizados','evento','status','observaciones'],
        'etapasdelote': ['codigo_lote','genero', 'operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','fecha','codigo_padre','codigo_madre','evento','status','observaciones'],
        'ccalidadpolen': ['codigo_lote','genero', 'operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','fecha','calidad','consistencia','ag_externos','status','observaciones'],
        'indexpolinizacion': ['codigo_lote','genero', 'operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','fecha','diasemana','color_lana','cantidad_camas','cantidad_index','cama1','cama2','cama3','cama4','cama5','media','total_index','status','observaciones'],
        'floresabiertas': ['codigo_lote','genero', 'operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','fecha','diasemana','nsemana','flores_abiertas','flores_anteras','flores_polinizadas','flores_sinpistilo','flores_viejas','boton_pequeño','status','observaciones'],
        'controlcosecha': ['codigo_lote','genero', 'operario_name','codigo_empleado','supervisor_name','ubicacion_lote','estructura','apodo_variedad','tipo_cultivo','fecha','cajas_revisadas','frutos_autopol','frutos_sinmarca','frutos_sinlana','frutos_fueratipo','llenado_caja','punto_maduracion','status','observaciones']
    }
...
def obtener_datos_empleado_post(request):
    if request.method == "POST":
        ...
        try:
            empleado = operariosApp.objects.exclude(status='Anulado').get(codigo_empleado=codigo)
            data = {
